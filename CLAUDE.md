...
- `doc_search.py` — keyword search over IDA HTML docs only; cross-links to `code_search` for related examples
- `code_search.py` — unified Python-source search: AST-chunked library APIs (`python/ida_*.py`, `idautils.py`, `idc.py`, `idalib/python/idapro/*.py`) plus example scripts (`python/examples/`, `idalib/examples/`). One result list with `kind="library"|"example"`. Library entries weight docstrings 3x; cross-links to `doc_search` for related HTML docs.
...
- `indirect_branch/` — indirect-branch site enumeration + LLM resolution recording. `scan.py` finds sites via `ida_idp.is_call_insn` + basic-block `fcb_indjump`. `persist.py` formats the `@RESOLVED_V1` comment block and manages manual code xrefs. `microcode.py` adds Hex-Rays-microcode evidence at `MMAT_GLBOPT3`: backward slice on the target operand, "from argN" inference via the function's tinfo + `reg2mreg`, inferred prototype, and caller-arg candidate enumeration. `arch/` holds per-arch helpers selected by `arch.get_helper()`; `arch/base.py` defines the no-op hook surface, `arch/arm64e.py` extracts the PAC discriminator from BLRAA/BLRAB/BRAA/BRAB. `api.py` wires the three MCP tools.
