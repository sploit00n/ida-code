...
- **Indirect-branch real-world fixes (Pass 4)** — shaken out by running the tools against AppleEmbeddedUSB.kext (273 functions, heavy IOKit C++):
  1. `scan_all` skips functions inside the `__auth_stubs` segment (PAC trampolines whose targets IDA already knows via the import table) — cut ~30% of noise on AppleEmbeddedUSB.kext.
  2. `microcode._locate_icall` falls back to fuzzy EA matching (±16 bytes) when Hex-Rays shifts the m_icall's reported EA after PAC normalization. On IDA 9.2 the BLRAA at raw 0x431c surfaces as an m_icall at 0x4324.
  3. `microcode` now emits a `{source: "vtable", vtable, offset, target_name?, target_addr?}` candidate when the icall's target operand is a load through a `__ZTV*` global — resolves C++ virtual dispatch without caller analysis.
  4. `arm64e.pac_discriminator` reconstructs the discriminator across `MOV X17, X_dyn ; MOVK X17, #imm, LSL #n` sequences and now returns `{value, value_mask, kind: "imm"|"movk"|"zero"|"dynamic"}` so callers can distinguish fully-known from partially-known values.
